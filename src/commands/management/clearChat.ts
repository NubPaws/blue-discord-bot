...
export class ClearChatCommand extends Command {
  constructor() {
    super('clear_chat', 'Deletes the specified number of messages from the channel.');
  }
  ...
  public async execute(message: Message, args: string[]): Promise<CommandResponse> {
    ...
    if (isNaN(amount) || amount <= 0) {
      return CommandResponse.message('Please specify a valid number of messages to delete.');
    }
    ...
    if (message.channel instanceof TextChannel) {
      ...
      return CommandResponse.message('This command can only be used in text channels.');
    }
  }
